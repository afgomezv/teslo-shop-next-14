...
export const paypalCheckPayment = async (paypalTransactionId: string) => {
  ...
  if (!authToken) {
    return {
      ok: false,
      message: "No se puede obtener el token",
    };
  }
  ...
  if (!response) {
    return {
      ok: false,
      message: "No se puede verificar el pago",
    };
  }
  ...
  if (status !== "COMPLETED") {
    return {
      ok: false,
      message: "El pago no se ha completado Paypal",
    };
  }
  ...
  try {
    ...
    return {
      ok: false,
      message: "500 - No se puede actualizar la base de datos",
    };
  }
};
